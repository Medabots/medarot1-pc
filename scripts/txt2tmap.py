...
if __name__ == '__main__':
    ...
    tilemap_table = sys.argv[3]

    fname = os.path.splitext(os.path.basename(input_file))[0]
    char_table = utils.reverse_dict(utils.merge_dicts([tilesets.get_tileset(tbl) for tbl in filter(None, utils.read_table(tilemap_table, keystring=True)[fname].split(","))]))
